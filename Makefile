...
create-firstime-config:
	@echo "Creating kubernetes configmap at: ./artifcats/configmap.yaml"
	@cp artifacts/configmap-template.yaml artifacts/configmap.yaml
	VERSION=$(VERSION) ./hack/generate.sh artifacts/configmap.yaml
	@kubectl --context=$(KUBE_CONTEXT) create -f artifacts/configmap.yaml
	@kubectl --context=$(KUBE_CONTEXT) get cm -n central | grep sqsmv
...
deploy:
	...
	@kubectl --context=$(KUBE_CONTEXT) apply -f artifacts/deployment.yaml
